...
let compoundObj0 = new CompoundClass({ foo: [x0, z], bar: { x: x1, b: b1 } });
let compoundObj1 = new CompoundClass({ foo: [x0, z], bar: { x: x1, b: b1 } });
const d = Provable.if(
   b1,
   CompoundType,
   compoundObj0,
 ...
 console.assert(d.bar.x.equals(x1).toBoolean());
...
let pubKey: PublicKey = PublicKey.fromPrivateKey(privKey);// privKey.toPublicKey();
